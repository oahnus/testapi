...
/**
 * Created by oahnus on 2017/9/18
 * 18:43.
 */
...
//    }
    ...
    private static void WriteToFile(File file, Workbook workBook) {
        try {
    ...
    public static void main(String[] args) throws IOException {
        String sheetName = "优惠券领取码表";
        String sheetTitle = "优惠券领取码表";
        List<String> columnNames = new LinkedList<>();
        columnNames.add("优惠券名称");
        columnNames.add("优惠券类型");
        columnNames.add("优惠开始时间");
        columnNames.add("优惠结束时间");
        columnNames.add("优惠产品");
        columnNames.add("加时/减价");
        columnNames.add("优惠码");

        //写入标题--第二种方式
        writeExcelTitle("src/main/resources/excel", "coupon", sheetName, columnNames, sheetTitle, false);
        ...
        for (int i = 0; i < 20; i++) {
            ...
            dataA.add("公司注册");
            dataA.add("500");
            dataA.add("aenadf0324oa2304jaof9234");
            objects.add(dataA);
        }
        try {
            //写入数据--第二种方式
            writeExcelData("src/main/resources/excel", "coupon", sheetName, objects);

            //直接写入数据--第一种方式
//            writeExcel("E:\\temp", "a", sheetName, columnNames, sheetTitle, objects, false);
        } catch (Exception e) {
            e.printStackTrace();
        }
